<script lang="ts">
    ...
    const profilePic = "/nopicture_placeholder.png";
    let labelState = 'open';
    $: if (!sidebarOpen) {
      labelState = 'closing';
  ...
  <div class="flex h-screen">
    ...
    <aside
      class={`transition-all duration-500 ease-in-out bg-gray-50 shadow h-full flex flex-col
        ${sidebarOpen ? 'w-[250px]' : 'w-[90px]'} rounded-r-3xl py-6 px-2`}
      onmouseenter={() => (sidebarOpen = true)}
      onmouseleave={() => (sidebarOpen = false)}
    >
      <!-- Top: AudioBrew Logo + Text -->
      <div class="flex items-center h-16 mb-10">
        <span class="flex items-center min-w-[70px] w-[70px] justify-center">
          <img src="/audiobrew_logo.png" alt="AudioBrew Logo" class="w-[80px] h-[60px] object-contain" />
        </span>
      ...
      <nav class="flex-1 flex flex-col gap-4">
        <!-- Dashboard -->
        <a href="/dashboard" class="flex items-center gap-2 px-3 py-3 hover:bg-gray-300 rounded-lg transition">
          <span class="flex items-center justify-center min-w-[40px] w-[40px] h-8">
            <!-- Dashboard Icon -->
            <svg width="22" height="22" viewBox="0 0 32 32" fill="none"><rect x="3" y="3" width="10" height="10" stroke="black" stroke-width="2" fill="none"/><rect x="19" y="3" width="10" height="10" stroke="black" stroke-width="2" fill="none"/><rect x="3" y="19" width="10" height="10" stroke="black" stroke-width="2" fill="none"/><rect x="19" y="19" width="10" height="10" stroke="black" stroke-width="2" fill="none"/></svg>
          </span>
          <span class={`sidebar-label${sidebarOpen ? ' open' : ''}${!sidebarOpen && labelState === 'closing' ? ' closing' : ''}`}>Dashboard</span>
        </a>
        <!-- Profile -->
        <a href="/profile" class="flex items-center gap-2 px-3 py-3 hover:bg-gray-300 rounded-lg transition">
          <span class="flex items-center justify-center min-w-[40px] w-[40px] h-8">
            <!-- Symmetrical Profile Icon -->
            <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" xmlns="http://www.w3.org/2000/svg"><circle cx="12" cy="8" r="4"/><path d="M4 20c0-4 8-4 8-4s8 0 8 4"/></svg>
          </span>
          <span class={`sidebar-label${sidebarOpen ? ' open' : ''}${!sidebarOpen && labelState === 'closing' ? ' closing' : ''}`}>Profile</span>
        </a>
        <!-- Settings -->
        <a href="/dashboard/settings" class="flex items-center gap-2 px-3 py-3 hover:bg-gray-300 rounded-lg transition">
          <span class="flex items-center justify-center min-w-[40px] w-[40px] h-8">
            <!-- Settings Icon (user SVG) -->
            <svg class="feather feather-settings" fill="none" height="22" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" viewBox="0 0 24 24" width="22" xmlns="http://www.w3.org/2000/svg"><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 0 1 0 2.83 2 2 0 0 1-2.83 0l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-2 2 2 2 0 0 1-2-2v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 0 1-2.83 0 2 2 0 0 1 0-2.83l.06-.06a1.65 1.65 0 0 0 .33-1.82 1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1-2-2 2 2 0 0 1 2-2h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 0 1 0-2.83 2 2 0 0 1 2.83 0l.06.06a1.65 1.65 0 0 0 1.82.33H9a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 2-2 2 2 0 0 1 2 2v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 0 1 2.83 0 2 2 0 0 1 0 2.83l-.06.06a1.65 1.65 0 0 0-.33 1.82V9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 2 2 2 2 0 0 1-2 2h-.09a1.65 1.65 0 0 0-1.51 1z"/></svg>
          </span>
          <span class={`sidebar-label${sidebarOpen ? ' open' : ''}${!sidebarOpen && labelState === 'closing' ? ' closing' : ''}`}>Settings</span>
        </a>
        <!-- Log out -->
        <button type="button" onclick={signOut} class="flex items-center gap-2 px-3 py-3 hover:bg-gray-300 rounded-lg transition">
          <span class="flex items-center justify-center min-w-[40px] w-[40px] h-8">
            <!-- Logout Icon (user SVG) -->
            <svg class="feather feather-log-out" fill="none" height="22" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" viewBox="0 0 24 24" width="22" xmlns="http://www.w3.org/2000/svg"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" x2="9" y1="12" y2="12"/></svg>
          </span>
      ...
      <div class="mt-auto flex flex-row items-center gap-2 pb-4 pl-4">
        <span class="flex items-center min-w-[40px] w-[40px] justify-center">
          <img src={profilePic} alt="ProfilePicture" class="w-8 h-8 rounded-full object-cover" />
        </span>
